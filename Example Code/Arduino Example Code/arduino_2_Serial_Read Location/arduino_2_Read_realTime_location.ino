...
** 2. The serial port monitor can be observed to output a position value location = xxxxx every second;
...
** 2. Set the motor working mode to Pulse+Direction Closed-loop FOC mode or Pulse+Pulse Closed-loop FOC mode, do not enable the motor (the motor shaft can be rotated by hand);
